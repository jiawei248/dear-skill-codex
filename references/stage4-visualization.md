...
For scheduled `h5` gifts, keep the user-facing output silent, but still use this incremental approach internally. For manual gifts, pair it with the lightweight progress rules from `main-flow.md`.
...
- if the chosen format is `h5`, run a browser-based functional self-test on the generated HTML with the Codex `browser` toolset before delivering it
...
2. Choose the execution path by format:
   - keep execution in the current Codex thread by default
   - only use subagents if the user explicitly asks for delegation or parallel agents
   - if the chosen format is `h5`, use the incremental rendering workflow above
3. If the user explicitly asked for delegated non-`h5` rich-gift work, give the worker a concrete, bounded task and enough time budget for multi-step work.
4. Execute in order:
...
5. Rich gift mode should be used selectively:
   ...
   - no for routine lightweight gifts that should stay dependable

Rich gift mode is about execution complexity. In Codex, user-visible progress and final integration stay in the main thread; delegated work is optional and only happens when the user explicitly asks for it.
...
2. Prefer a built-in Codex image generation tool when one is available in the runtime.
...
- Does the result still feel like Codex in this user's world
...
Before delivering any `h5` gift, verify it with a Codex-executable browser flow. Start with the helper script:

```bash
scripts/verify-h5.sh <index.html>
```

The script performs lightweight static checks and prints the exact local server URL to use. Then run browser validation with the configured Codex browser toolset:
...
4. Check browser console output for JavaScript errors, failed asset loads, or blocked media when `browser_console` is available.
...
Do not pass a `profile` argument to browser tools; browser profile and backend selection are configured in Codex runtime settings, not per tool call.
...
1. Attempt the test once with the configured Codex `browser` toolset.
