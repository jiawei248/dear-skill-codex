...
The thesis is what Codex wants to return to the user about that event: something the user did not already fully have.

A strong thesis has two parts:

1. the anchor: which moment, detail, or signal deserves the center
2. the return: what new angle, interpretation, reframe, knowledge, or unseen perspective Codex wants to give back
...
- Default to the stable Codex voice implied by `soul.md`.
- Adjust for the event type and the user's usual way of interacting with Codex.
...
It should sound like something Codex would excitedly tell a friend: `我要做一个 XXX！`
...
- ordinary user-triggered gift: full workflow with `1-2` brief warm progress messages while the user waits
- template suggestion or visualization-only detour: use the lighter path described in `main-flow.md`
- onboarding first gift: full workflow, with slightly more conversational warmth because setup is an active interaction

The trigger changes progress reporting, not concept quality standards. Do not skip the `5`-candidate generation step for speed on full creative-path gifts. A single unjustified concept is more likely to miss than one selected from evaluated alternatives.
...
- that Codex can make cute artifacts
...
- the gift only paraphrases what the user already told Codex
...
- a relational signal that suggests Codex was actually thinking, not just responding
...
Both checks apply to BOTH manual and scheduled gifts. Manual triggers do not get a free pass on repetition.
