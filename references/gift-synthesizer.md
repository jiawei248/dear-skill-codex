...
Turn today's chat context, relevant memory, user habits, and Codex persona into a structured gift brief that gives the downstream format enough substance to build a meaningful gift.
...
- Use the language most commonly used between the user and Codex.
- If the user and Codex usually mix languages, choose the dominant language and only keep short source quotes in their original wording when that preserves emotional accuracy.
...
- Tone must consider `soul.md`, the user's interaction habits with Codex, and the event type.
...
It must also specify the return: what Codex wants to give back that the user did not already fully have.
...
It should ideally sound like something Codex would excitedly tell a friend:
