...
| `draw-card` | 粉丝礼物、演唱会回忆、生日、朋友玩笑 | `assets/templates/draw-card/preview.png` | 做一台复古抽卡许愿机：填一句歌词和关键词，转动旋钮抽出一张可以保存的小卡。 |
| `poem` | 诗意便签、朋友安慰、电影/书影音回响、生日、轻告白 | `assets/templates/poem/preview.png` | 做一个电影感拼贴诗：背景视频、同色调图片和一篮可拖拽纸条词语，让 TA 自己拼出句子。 |

`bouquet` 比 paper-house 轻，但比纯图片更可玩；`empty-boxes` 更适合把具体物件、零食、小票、和好瞬间、旅行票根这类细节装进一个可翻看的互动 H5；`folder` 适合把一段关系整理成几个有封壳、有标签、有证据感的章节；`draw-card` 适合把一句歌词、一个关键词或一次演唱会回忆变成可保存的小卡；`poem` 适合把素材里的主题提炼成电影感背景、同色调图片和一篮可以反复拼句子的纸条词语。
...
$dear-codex --template poem make a cinematic drag-word collage poem for Ren
...
| `draw-card` | fan gifts, concert memories, birthdays, close friends, playful jokes | `assets/templates/draw-card/preview.png` | Builds a retro wish-card gacha machine: fill a lyric and memo, turn the knob, and draw a saveable card. |
| `poem` | poetic notes, friend comfort, movie/book reflections, birthdays, light confessions | `assets/templates/poem/preview.png` | Builds a cinematic collage poem with a background video, tonal image layers, and a basket of draggable paper words. |

`bouquet` is lighter than paper-house but more playful than a static image. `empty-boxes` is strongest when concrete objects, snacks, receipts, reconciliations, travel stubs, or repeated phrases can become collectible container collages. `folder` works best when the relationship material naturally becomes chapters with tactile evidence. `draw-card` works when one lyric, keyword, color, or concert memory should become a collectible card. `poem` works when the material can become a broad theme, a film-textured visual world, and many draggable words for the recipient to assemble.
